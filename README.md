...
3. ~~**Favicon.**~~ **Done.** Replaced the wordmark `logo.png` favicon with `docs/favicon.png` — a 512×512 square of the brand heart (centered, padded). Linked via `<link rel="icon" type="image/png" sizes="512x512">`. (Optional later: an `apple-touch-icon.png` at 180×180 on a **solid** background — iOS composites transparency onto black, so the current transparent heart would disappear on a home screen. The transparent favicon is also faint on dark-mode browser tab strips; add a solid tile if that matters.)
