...
- **Analytics (GA4) + consent:** Google Analytics (`G-2MM4MWELZG`) is **consent-gated** — `gtag.js` is not loaded and nothing is sent to Google until the visitor clicks **Accept** on the bottom consent banner (`#consent`). The choice is stored in `localStorage` (`gf.consent` = `granted`/`denied`); the banner only shows when no choice has been made. Being a SPA, navigations fire manual `page_view` events via `window.__gfTrackPage()` (called from `openDetail`/`dropWineParam`/`popstate` after the URL changes); the initial `gtag('config')` covers the first load. The banner is bilingual (`data-en`) and sits behind the age gate, so first-time visitors see it after passing the gate. To change the consent copy, edit `#consent`; to swap the GA property, edit `GA_ID`.
